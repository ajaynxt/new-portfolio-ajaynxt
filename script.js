(() => {
  ...
  if (intro) {
    ...
    const finishIntro = () => {
      ...
      window.setTimeout(() => {
        if (intro.classList.contains('is-finished')) intro.remove();
      }, reducedMotion ? 20 : 780);
    };
    const introDelay = reducedMotion ? 260 : 1850;
    window.setTimeout(finishIntro, introDelay);
    window.addEventListener('pageshow', () => {
      if (document.visibilityState === 'visible' && !intro.classList.contains('is-finished')) {
        window.setTimeout(finishIntro, reducedMotion ? 120 : 1750);
      }
    }, { once: true });
  }
