(() => {
  ...
  let flowPaletteIndex = 0;

  const paintFlowPalette = () => {
    if (document.body.classList.contains('flow-off') || !flowBlobs.length) return;
    const palette = flowPalettes[flowPaletteIndex % flowPalettes.length];
    flowBlobs.forEach((blob, index) => {
      blob.style.setProperty('--blob-color', palette[index % palette.length]);
    });
    flowPaletteIndex += 1;
  };

  paintFlowPalette();
  window.setInterval(paintFlowPalette, 1000);
